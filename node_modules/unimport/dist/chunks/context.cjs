...
async function scanDirExports(dir, options) {
  ...
  const files = await fg__default(dirs.flatMap((i) => [
    i,
    ...filePatterns.map((p) => pathe.join(i, p))
  ]), {
    absolute: true,
    cwd: options?.cwd || process.cwd(),
    onlyFiles: true,
    followSymbolicLinks: true
  }).then((r) => r.map((f) => pathe.normalize(f)).sort().filter(fileFilter));
  const imports = [];
...
function createUnimport(opts) {
  ...
  return {
    ...
    toExports: (filepath) => vueTemplate.toExports(ctx.imports, filepath),
...
async function detectImports(code, ctx, options) {
  ...
  if (options?.autoImport !== false) {
    const identifiers = new Set(Array.from(strippedCode.matchAll(vueTemplate.matchRE)).map((i) => {
      if (i[1] === ".") {
        return "";
      }
