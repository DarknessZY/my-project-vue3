...
import { normalize, join, parse } from 'pathe';
import { findExports, findStaticImports, parseStaticImport, detectSyntax } from 'mlly';
import { camelCase } from 'scule';
import { d as defineUnimportPreset, v as vueTemplateAddon, n as normalizeImports, b as dedupeImports, c as toExports, j as getMagicString, a as stripCommentsAndStrings, m as matchRE, e as excludeRE, s as separatorRE, i as importAsRE, k as addImportToCode, g as toTypeDeclarationFile } from './vue-template.mjs';
...
async function scanDirExports(dir, options) {
  ...
  const files = await fg(dirs.flatMap((i) => [
    i,
...
function createUnimport(opts) {
  ...
  return {
    ...
    toExports: (filepath) => toExports(ctx.imports, filepath),
...
async function detectImports(code, ctx, options) {
  ...
  if (options?.autoImport !== false) {
    const identifiers = new Set(Array.from(strippedCode.matchAll(matchRE)).map((i) => {
      if (i[1] === ".") {
        return "";
      }
    ...
    matchedImports = Array.from(identifiers).map((name) => ctx.map.get(name)).filter((i) => i && !i.disabled);
    for (const addon of ctx.addons) {
  ...
  if (options?.transformVirtualImoports !== false && ctx.options.virtualImports?.length) {
    ...
    virtualImports.forEach((i) => {
      ...
      Object.entries(i.namedImports || {}).forEach(([name, as]) => {
        const original2 = ctx.map.get(name);
        if (!original2) {
