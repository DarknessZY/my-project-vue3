...
function rewriteSlot(node, context) {
    ...
    if (hasOtherDir) {
        context.onError((0, compiler_core_1.createCompilerError)(36 /* X_V_SLOT_UNEXPECTED_DIRECTIVE_ON_SLOT_OUTLET */, node.loc));
    }
    if (nonNameProps.length > 0) {
        (0, transformElement_1.processProps)(node, context, nonNameProps);
        const properties = [];
        nonNameProps.forEach((prop) => {
            if (prop.type === 7 /* DIRECTIVE */ && prop.name === 'bind') {
...
function transformScopedSlotName(node, context) {
    ...
    if (nameProps && nameProps.type === 6 /* ATTRIBUTE */ && nameProps.value) {
        const { props } = node;
        props.splice(props.indexOf(nameProps), 1, (0, uni_cli_shared_1.createBindDirectiveNode)('name', (0, utils_1.rewriteExpression)((0, compiler_core_1.createSimpleExpression)(`"${nameProps.value.content}-"+` + slotKey), context).content));
    }
}
...
function transformProperty(dir, context) {
    if (!dir.arg || !dir.exp) {
        return;
    }
    const isStaticArg = dir.arg.type === 4 /* SIMPLE_EXPRESSION */ && dir.arg.isStatic;
    if (isStaticArg) {
