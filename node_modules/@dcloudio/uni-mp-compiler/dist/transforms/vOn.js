...
const transformOn = (dir, node, _context, augmentor) => {
    ...
    if (!dir.exp && !modifiers.length) {
        context.onError((0, compiler_core_1.createCompilerError)(35 /* X_V_ON_NO_EXPRESSION */, loc));
    }
    let eventName;
...
function isFilterExpr(value, context) {
    if (context.filters.length && value.type === 8 /* COMPOUND_EXPRESSION */) {
        const firstChild = value.children[0];
        if (firstChild.type === 4 /* SIMPLE_EXPRESSION */ &&
...
function wrapperVOn(value, node, context) {
    ...
    const keys = [];
    if (((_a = context.miniProgram.event) === null || _a === void 0 ? void 0 : _a.key) && context.inVFor) {
        const keyProp = (0, compiler_core_1.findProp)(node, 'key');
